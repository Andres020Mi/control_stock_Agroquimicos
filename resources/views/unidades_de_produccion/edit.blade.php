...
@section('content')
    <div class="container mx-auto py-8">
        <h1 class="text-3xl font-bold text-green-800 mb-6">Editar Unidad de Producción</h1>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-300">
            <form action="{{ route('unidades_de_produccion.update', $unidad->id) }}" method="POST" class="update-form">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="nombre" id="nombre" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" value="{{ old('nombre', $unidad->nombre) }}" required>
                </div>

                <div class="mb-6">
                    <label for="descripcion" class="block text-sm font-semibold text-gray-700 mb-2">Descripción (Opcional)</label>
                    <textarea name="descripcion" id="descripcion" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" rows="3">{{ old('descripcion', $unidad->descripcion) }}</textarea>
                </div>
                ...
                <div class="flex space-x-4">
                    ...
                    <a href="{{ route('unidades_de_produccion.index') }}" class="px-6 py-3 bg-gray-500 text-white font-semibold rounded-lg shadow hover:bg-gray-600 transition duration-200">
                        Cancelar
                    </a>
                </div>
            </form>
