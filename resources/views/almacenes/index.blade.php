...
@section('content')
    <div class="container mx-auto py-8">
        <h1 class="text-3xl font-bold text-green-800 mb-6">Listado de Almacenes</h1>
        ...
        <div class="mb-6">
            <a href="{{ route('almacenes.create') }}" class="inline-block px-6 py-3 bg-green-700 text-white font-semibold rounded-lg shadow hover:bg-green-800 transition duration-200">
                Crear Nuevo Almacén
            </a>
        </div>
                ...
                <tbody class="bg-gray-50">
                    @forelse ($almacenes as $almacen)
                        <tr class="hover:bg-green-50 transition duration-150">
                            <td class="px-6 py-4 text-sm text-gray-800 border-t border-gray-200">{{ $almacen->id }}</td>
                            <td class="px-6 py-4 text-sm text-gray-800 border-t border-gray-200">{{ $almacen->nombre }}</td>
                            <td class="px-6 py-4 text-sm text-gray-800 border-t border-gray-200">{{ $almacen->descripcion ?? 'Sin descripción' }}</td>
                            <td class="px-6 py-4 text-sm border-t border-gray-200">
                                <a href="{{ route('almacenes.edit', $almacen->id) }}" class="inline-block px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition duration-200">
                                    Editar
                                </a>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-sm text-gray-600 text-center border-t border-gray-200">No hay almacenes registrados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.delete-form').on('submit', function(e) {
                    ...
                    Swal.fire({
                        title: '¿Estás seguro?',
                        text: '¿Quieres eliminar este almacén? Esto también eliminará los stocks asociados.',
                        icon: 'warning',
